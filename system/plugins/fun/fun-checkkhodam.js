/*
Thanks To BetaBotz Form Database Khodam❤😘❤😘
*/
    ...
    async run(m, { text }) {
        if (!text) {
            return m.reply("╭════════════════════\n" +
                         ...
                         "│ Contoh: .cekkhodam Raiden\n" +
                         "╰════════════════════");
        }

        try {
            const khodamData = await khodamnya();
            
            const caption = `╭═══❯ *KHODAM CHECK* ❮═══
│ 
│ 👤 *Nama:* ${text}
│ 📿 *Khodam:* ${khodamData.name}
│ ✨ *Meaning:* ${khodamData.meaning}
╰════════════════════

📝 *Catatan:* Setiap Khodam memiliki karakteristik unik yang dapat membantu pemiliknya dalam berbagai aspek kehidupan. Gunakan kebijaksanaan dalam memanfaatkan kekuatan ini.

🌟 *Tip:* Lakukan meditasi dan ritual pembersihan energi secara rutin untuk memperkuat ikatan dengan Khodam Anda.`;

            await m.reply("⌛ *Membuka Portal Dimensi Khodam...*");
            await new Promise(resolve => setTimeout(resolve, 2000));
            await m.reply("🔮 *Menganalisis Energi Spiritual...*");
            await new Promise(resolve => setTimeout(resolve, 1500));
            
            await m.reply(caption);

        } catch (error) {
            console.error(chalk.red('Error in cekkhodam command:', error));
            return m.reply("╭══════════════════════\n" +
                         "│ ❌ *Terjadi Kesalahan*\n" +
